...
final class ConfigFileManager {
	...
	public static function saveCache(string $filename, string $content) {
		...
		if ($response === false) {
			throw new IOException("Unable to create cache file $filename");
		}
	}
	...
	public static function loadDocumentNode(string $schemaFilePath): Node {
		if (!is_readable($schemaFilePath)) {
			throw new IOException("Unable to access schema file $schemaFilePath");
		}
	...
	public static function loadTypes(string $dir, string $baseNamespace): \Generator {
		...
		while (($file = readdir($handle)) !== false) {
			...
			if (is_dir($dir.DIRECTORY_SEPARATOR.$file)) {
				yield from self::loadTypes($dir.DIRECTORY_SEPARATOR.$file, $class);
			} else {
				yield $class;
			}
		}
